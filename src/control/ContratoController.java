...
public class ContratoController {
		...
		// Verifica se a renda esta no perfil B
		{
			...
			if (renda <= valorMaxPerfilB) {
				...
				if (renda <= valorMaxPerfilC) {
					...
					// Se a rendar n�o estiver em nenhum dos outros, ela �
					// do
					// tipo D
	...
	// Retorna se o perfil do cliente est� aprovado ou reprovado
	...
	public double calculaValorParcelas(int numeroParcelas, double valorEmprestimo, PlanoEmprestimo plano) {
		return ((valorEmprestimo * (1 + plano.getJurosTotal()))
				/ (1 - Math.pow((1 + plano.getJurosMensal()), -numeroParcelas)) / plano.getJurosMensal());
	...
	public void salvarContrato(int idCliente, String status, int idPlanoEmprestimo, int numParcelas, double valorEmprestimo, double valorParcelas, String dataTermino, String observacoes) {
		...
		Contrato contrato = new Contrato(numParcelas, valorEmprestimo, valorParcelas,
				new java.sql.Date(Calendar.getInstance().getTime().getTime()), this.formatDate(dataTermino), clt,
				status, plEmprestimo, observacoes);
		...
		if (persistidoSucesso) {
			...
			InterfaceUsuario.exibirMensagemContratoCadastro("N�o foi poss�vel cadastrar o contrato.");
		}
	}

	public void editarContrato(int codContrato, int idCliente, String status, int idPlanoEmprestimo, int numParcelas,
